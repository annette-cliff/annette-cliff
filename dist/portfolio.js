...
const portfolioGridHTML = portfolioContent
  .map(item => `<div class="grid-item" tabindex="0" data-trigger-modal="${item.title.toLowerCase().replace(/ +/g, '-').replace('&', 'and')}">
                    ...
                    <picture>
                      ...
                      <img src="${item.desktop_image}" alt="${item.title}'s homepage image on desktop" width="2880" height="1360" class="desktop-only">
                    </picture>
                    <picture>
                      <source type="image/webp" srcset="${item.mobile_image}" media="(min-width: 600px)" />
                      <img src="${item.mobile_image}" alt="${item.title}'s homepage image on mobile" width="1110" height="1356" class="mobile-only">
                    </picture>
                </div>`)
  .join('');
